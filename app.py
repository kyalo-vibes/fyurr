...
#Run the first time to create tables in the DB, then SQLAlchemy takes over from there
#db.create_all()
...
def show_venue(venue_id):
  ...
  data1={
    ...
    "searching_venue": True,
    "desc": "We are on the lookout for a local artist to play every two weeks. Please call us.",
...
def edit_artist_submission(artist_id):
  ...
  if form.validate():
    ...
    # e.g., flash('An error occurred. Artist ' + data.name + ' could not be listed.')
      ...
      flash('Artist ' + request.form['name'] + ' could not be changed')
      db.session.rollback()
    finally:
      db.session.close()
  else:
...
def edit_venue_submission(venue_id):
  ...
  if form.validate():
    ...
    # e.g., flash('An error occurred. Artist ' + data.name + ' could not be listed.')
      ...
      flash('Venue ' + request.form['name'] + ' could not be changed')
      db.session.rollback()
    finally:
      db.session.close()
  else:
